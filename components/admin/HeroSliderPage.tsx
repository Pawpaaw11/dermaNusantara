...
export function HeroSliderPage(){
 ...
 function open(item?:HeroSlide){setMessage(undefined);setEditing(item??"new");setForm(item?{desktopImageUrl:item.desktopImageUrl,desktopImageAlt:item.desktopImageAlt,mobileImageUrl:item.mobileImageUrl??"",mobileImageAlt:item.mobileImageAlt??"",linkUrl:item.linkUrl??"",isActive:item.isActive}:blank)}
 ...
 return <section><PageHeader title="Hero Slider" description="Atur banner utama homepage, urutan, status, serta gambar desktop dan mobile." action={<button className="admin-button admin-button-primary" onClick={()=>open()}><Plus size={17}/>Tambah slider</button>}/>
  ...
  {editing&&<form onSubmit={submit} className="mb-6 space-y-5 rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="font-bold text-primary">{editing==="new"?"Tambah hero slider":"Edit hero slider"}</h2><p className="text-sm text-slate-500">Gambar mobile bersifat opsional dan otomatis memakai gambar desktop jika kosong.</p></div><button type="button" className="admin-button" onClick={()=>setEditing(undefined)}><X size={16}/>Tutup</button></div>
   ...
   <button disabled={save.isPending} className="admin-button admin-button-primary"><Save size={17}/>{save.isPending?"Menyimpan…":"Simpan slider"}</button>
  </form>}
